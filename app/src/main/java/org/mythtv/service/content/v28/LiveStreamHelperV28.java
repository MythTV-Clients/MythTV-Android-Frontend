 ...
 * 
 */
package org.mythtv.service.content.v28;
 ...
 *
 */
public class LiveStreamHelperV28 extends AbstractBaseHelper {
	...
	private static LiveStreamHelperV28 singleton;
	
	/**
	 * Returns the one and only LiveStreamHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static LiveStreamHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( LiveStreamHelperV28.class ) {

				if( null == singleton ) {
					singleton = new LiveStreamHelperV28();
				}
	...
	public boolean create( final Context context, final LocationProfile locationProfile, final Integer channelId, final DateTime startTime ) {
		...
		if( null != selectedPlaybackProfile ) {
			...
			try {
				Program program = RecordedHelperV28.getInstance().findRecorded( context, locationProfile, channelId, startTime );
	...
	public Integer load( final Context context, final LocationProfile locationProfile ) {
		...
		try {
			...
			ResponseEntity<LiveStreamInfoList> wrapper = mMythServicesTemplate.contentOperations().getLiveStreamList( null, EtagInfoDelegate.createEmptyETag() );
			if( wrapper.getStatusCode().equals( HttpStatus.OK ) ) {
	...
	public boolean deleteLiveStream( final Context context, final Long id ) {
		...
		if( null == context ) 
			throw new RuntimeException( "LiveStreamHelperV28 is not initialized" );
