 ...
 *
 */
public class RecordingHelperV25 extends AbstractBaseHelper {

	private static final String TAG = RecordingHelperV25.class.getSimpleName();
	
	private static RecordingHelperV25 singleton;
	
	/**
	 * Returns the one and only RecordingHelperV25. init() must be called before 
	 * any 
	 * @return
	 */
	public static RecordingHelperV25 getInstance() {
		if( null == singleton ) {
			
			synchronized( RecordingHelperV25.class ) {

				if( null == singleton ) {
					singleton = new RecordingHelperV25();
				}
