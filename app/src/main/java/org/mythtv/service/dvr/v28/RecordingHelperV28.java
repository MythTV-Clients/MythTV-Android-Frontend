 ...
 *
 */
public class RecordingHelperV28 extends AbstractBaseHelper {

	private static final String TAG = RecordingHelperV28.class.getSimpleName();
	
	private static RecordingHelperV28 singleton;
	
	/**
	 * Returns the one and only RecordingHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static RecordingHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( RecordingHelperV28.class ) {

				if( null == singleton ) {
					singleton = new RecordingHelperV28();
				}
