 ...
 *
 */
public class RecordingRuleHelperV28 extends AbstractBaseHelper {
	...
	private static RecordingRuleHelperV28 singleton;
	
	/**
	 * Returns the one and only RecordingRuleHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static RecordingRuleHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( RecordingRuleHelperV28.class ) {

				if( null == singleton ) {
					singleton = new RecordingRuleHelperV28();
				}
	 ...
	 */
	private RecordingRuleHelperV28() { }
	...
	private int load( final Context context, final LocationProfile locationProfile, final org.mythtv.services.api.v027.beans.RecRule[] recordingRules ) throws RemoteException, OperationApplicationException {
		...
		if( null == context ) 
			throw new RuntimeException( "RecordingRuleHelperV28 is not initialized" );
