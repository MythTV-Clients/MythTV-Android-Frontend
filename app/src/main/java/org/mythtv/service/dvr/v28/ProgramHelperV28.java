 ...
 *
 */
public class ProgramHelperV28 extends AbstractBaseHelper {
	...
	private static ProgramHelperV28 singleton;
	
	/**
	 * Returns the one and only ProgramHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static ProgramHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( ProgramHelperV28.class ) {

				if( null == singleton ) {
					singleton = new ProgramHelperV28();
				}
		...
		if( null != response ) {
			...
			if( response.getStatusCode().equals( HttpStatus.OK ) ) {
				...
				if( removed ) {
					...
					if( deleted == 1 ) {
						RecordingHelperV28.getInstance().deleteRecording( context, locationProfile, RecordingConstants.ContentDetails.getValueFromParent( table ).getContentUri(), recordId, recordingStartTime );
					}
