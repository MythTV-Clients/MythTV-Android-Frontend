 ...
 * 
 */
package org.mythtv.service.guide.v28;
 ...
 *
 */
public class ProgramGuideHelperV28 extends AbstractBaseHelper {
	...
	private static ProgramGuideHelperV28 singleton;
	
	/**
	 * Returns the one and only ProgramGuideHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static ProgramGuideHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( ProgramGuideHelperV28.class ) {

				if( null == singleton ) {
					singleton = new ProgramGuideHelperV28();
				}
	...
	public static Program findProgram( final Context context, final LocationProfile locationProfile, Integer channelId, DateTime startTime ) {
		...
		Program program = ProgramHelperV28.getInstance().findProgram( context, locationProfile, ProgramConstants.CONTENT_URI_GUIDE, ProgramConstants.TABLE_NAME_GUIDE, channelId, startTime );
	...
	public static boolean deleteProgram( final Context context, final LocationProfile locationProfile, Integer channelId, DateTime programStartTime, DateTime recordingStartTime, Integer recordId ) {
		...
		boolean removed = ProgramHelperV28.getInstance().deleteProgram( context, locationProfile, ProgramConstants.CONTENT_URI_GUIDE, ProgramConstants.TABLE_NAME_GUIDE, channelId, programStartTime, recordingStartTime, recordId );
	...
	private static void downloadProgramGuide( final Context context, final LocationProfile locationProfile ) throws MythServiceApiRuntimeException, RemoteException, OperationApplicationException {
		...
		ProgramHelperV28.getInstance().deletePrograms( context, locationProfile, ops, ProgramConstants.CONTENT_URI_GUIDE, ProgramConstants.TABLE_NAME_GUIDE );
		RecordingHelperV28.getInstance().deleteRecordings( context, locationProfile, ops, ContentDetails.GUIDE );
	...
	private static int load( final Context context, final LocationProfile locationProfile, final ProgramGuide programGuide ) throws RemoteException, OperationApplicationException {
		...
		if( null == context ) 
			throw new RuntimeException( "ProgramGuideHelperV28 is not initialized" );
