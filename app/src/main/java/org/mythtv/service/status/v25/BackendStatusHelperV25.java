 ...
 * 
 */
package org.mythtv.service.status.v25;
...
import org.mythtv.service.dvr.v25.ProgramHelperV25;
import org.mythtv.service.dvr.v25.RecordingHelperV25;
 ...
 *
 */
public class BackendStatusHelperV25 extends AbstractBaseHelper {

	private static final String TAG = BackendStatusHelperV25.class.getSimpleName();
	...
	private static BackendStatusHelperV25 singleton;
	
	/**
	 * Returns the one and only BackendStatusHelperV25. init() must be called before 
	 * any 
	 * @return
	 */
	public static BackendStatusHelperV25 getInstance() {
		if( null == singleton ) {
			
			synchronized( BackendStatusHelperV25.class ) {

				if( null == singleton ) {
					singleton = new BackendStatusHelperV25();
				}
	 ...
	 */
	private BackendStatusHelperV25() { }
	...
	private void updateProgramGuide( final Context mContext, org.mythtv.services.api.v025.status.beans.BackendStatus status ) throws RemoteException, OperationApplicationException {
...
//					Log.v( TAG, "updateProgramGuide : versionProgram=" + versionProgram.toString() );
					...
					ProgramHelperV25.getInstance().processProgram( mContext, mLocationProfile, ProgramConstants.CONTENT_URI_UPCOMING, ProgramConstants.TABLE_NAME_UPCOMING, ops, convertV25Program( versionProgram ), tag );
					count++;
					
					// update program guide
					ProgramHelperV25.getInstance().processProgram( mContext, mLocationProfile, ProgramConstants.CONTENT_URI_GUIDE, ProgramConstants.TABLE_NAME_GUIDE, ops, convertV25Program( versionProgram ), tag );
					count++;
					...
					if( !inError && null != versionProgram.getRecording() ) {
						...
						if( versionProgram.getRecording().getRecordId() > 0 ) {
							...
							RecordingHelperV25.getInstance().processRecording( mContext, mLocationProfile, ops, RecordingConstants.ContentDetails.UPCOMING, convertV25Program( versionProgram ), tag );
							count++;

							// update program guide recording
							RecordingHelperV25.getInstance().processRecording( mContext, mLocationProfile, ops, RecordingConstants.ContentDetails.GUIDE, convertV25Program( versionProgram ), tag );
							count++;
...
//						Log.i( TAG, "load : applying batch for '" + count + "' transactions, processing programs" );
	...
	private BackendStatus convertBackendStatus( org.mythtv.services.api.v025.status.beans.BackendStatus status ) {
		...
		if( null != status.getEncoders() ) {
			...
			if( null != status.getEncoders().getEncoders() && !status.getEncoders().getEncoders().isEmpty() ) {
				...
				for( org.mythtv.services.api.v025.status.beans.Encoder versionEncoder : status.getEncoders().getEncoders() ) {
		...
		if( null != status.getJobQueue() ) {
			...
			if( null != status.getJobQueue().getJobs() && !status.getJobQueue().getJobs().isEmpty() ) {
				...
				for( org.mythtv.services.api.v025.status.beans.Job versionJob : status.getJobQueue().getJobs() ) {
					Job job = new Job();
					job.setArgs( versionJob.getArgs() );
					job.setChannelId( versionJob.getChannelId() );
					
					if( null != versionJob.getCommand() ) {
						org.mythtv.services.api.v025.status.beans.Job.Command versionCommand = versionJob.getCommand();
					...
					if( null != versionJob.getArgs() ) {
						org.mythtv.services.api.v025.status.beans.Job.Flag versionFlag = versionJob.getFlag();
					...
					if( null != versionJob.getStatus() ) {
						org.mythtv.services.api.v025.status.beans.Job.Status versionStatus = versionJob.getStatus();
						job.setStatus( Job.Status.valueOf( versionStatus.name() ) );
					}
					
					if( null != versionJob.getType() ) {
						org.mythtv.services.api.v025.status.beans.Job.Type versionType = versionJob.getType();
		...
		if( null != status.getMachineInfo() ) {
			...
			if( null != status.getMachineInfo().getStorage() ) {
				...
				if( null != status.getMachineInfo().getStorage().getGroups() && !status.getMachineInfo().getStorage().getGroups().isEmpty() ) {
					...
					for( org.mythtv.services.api.v025.status.beans.Group versionGroup : status.getMachineInfo().getStorage().getGroups() ) {
		...
		if( null != status.getMiscellaneous() ) {
			...
			if( null != status.getMiscellaneous().getInformations() && !status.getMiscellaneous().getInformations().isEmpty() ) {
				...
				for( org.mythtv.services.api.v025.status.beans.Information versionInfo : status.getMiscellaneous().getInformations() ) {
	...
	private Program convertProgram( org.mythtv.services.api.v025.status.beans.Program versionProgram ) {
		...
		if( null != versionProgram.getArtwork() ) {
			...
			if( null != versionProgram.getArtwork().getArtworkInfos() && !versionProgram.getArtwork().getArtworkInfos().isEmpty() ) {
				...
				for( org.mythtv.services.api.v025.status.beans.ArtworkInfo versionArtwork : versionProgram.getArtwork().getArtworkInfos() ) {
	...
	private org.mythtv.services.api.v025.beans.Program convertV25Program( org.mythtv.services.api.v025.status.beans.Program versionProgram ) {
		
		org.mythtv.services.api.v025.beans.Program program = new org.mythtv.services.api.v025.beans.Program();
		...
		if( null != versionProgram.getRecording() ) {
			program.setRecording( convertV25Recording( versionProgram.getRecording() ) );
		}
		
		if( null != versionProgram.getChannelInfo() ) {
			program.setChannel( convertV25Channel( versionProgram.getChannelInfo() ) );
		}
		
		if( null != versionProgram.getArtwork() ) {
			
			org.mythtv.services.api.v025.beans.ArtworkInfoList artworkInfos = new org.mythtv.services.api.v025.beans.ArtworkInfoList();
			
			if( null != versionProgram.getArtwork().getArtworkInfos() && versionProgram.getArtwork().getArtworkInfos().isEmpty() ) {
			
				List<org.mythtv.services.api.v025.beans.ArtworkInfo> artworkInfoList = new ArrayList<org.mythtv.services.api.v025.beans.ArtworkInfo>();
				
				for( org.mythtv.services.api.v025.status.beans.ArtworkInfo versionArtwork : versionProgram.getArtwork().getArtworkInfos() ) {
					artworkInfoList.add( convertV25Artwork( versionArtwork ) );
				}
				
				artworkInfos.setArtworkInfos( artworkInfoList.toArray( new org.mythtv.services.api.v025.beans.ArtworkInfo[ artworkInfoList.size() ] ) );
			}
	...
	private Recording convertRecording( org.mythtv.services.api.v025.status.beans.Recording versionRecording ) {
	...
	private org.mythtv.services.api.v025.beans.RecordingInfo convertV25Recording( org.mythtv.services.api.v025.status.beans.Recording versionRecording ) {
		
		org.mythtv.services.api.v025.beans.RecordingInfo recording = new org.mythtv.services.api.v025.beans.RecordingInfo();
	...
	private ChannelInfo convertChannel( org.mythtv.services.api.v025.status.beans.ChannelInfo versionChannel ) {
	...
	private org.mythtv.services.api.v025.beans.ChannelInfo convertV25Channel( org.mythtv.services.api.v025.status.beans.ChannelInfo versionChannel ) {
		
		org.mythtv.services.api.v025.beans.ChannelInfo channel = new org.mythtv.services.api.v025.beans.ChannelInfo();
	...
	private ArtworkInfo convertArtwork( org.mythtv.services.api.v025.status.beans.ArtworkInfo versionArtwork ) {
	...
	private org.mythtv.services.api.v025.beans.ArtworkInfo convertV25Artwork( org.mythtv.services.api.v025.status.beans.ArtworkInfo versionArtwork ) {
		
		org.mythtv.services.api.v025.beans.ArtworkInfo artworkInfo = new org.mythtv.services.api.v025.beans.ArtworkInfo();
		artworkInfo.setFileName( versionArtwork.getFilename() );
		artworkInfo.setStorageGroup( versionArtwork.getStorageGroup() );
		artworkInfo.setType( versionArtwork.getType() );
