 ...
 * 
 */
package org.mythtv.service.status.v28;
...
import org.mythtv.service.dvr.v28.ProgramHelperV28;
import org.mythtv.service.dvr.v28.RecordingHelperV28;
 ...
 *
 */
public class BackendStatusHelperV28 extends AbstractBaseHelper {
	...
	private static BackendStatusHelperV28 singleton;
	
	/**
	 * Returns the one and only BackendStatusHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static BackendStatusHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( BackendStatusHelperV28.class ) {

				if( null == singleton ) {
					singleton = new BackendStatusHelperV28();
				}
	 ...
	 */
	private BackendStatusHelperV28() { }
...
//					Log.v( TAG, "updateProgramGuide : versionProgram=" + versionProgram.toString() );
					...
					ProgramHelperV28.getInstance().processProgram( mContext, mLocationProfile, ProgramConstants.CONTENT_URI_UPCOMING, ProgramConstants.TABLE_NAME_UPCOMING, ops, convertV28Program( versionProgram ), tag );
					count++;
					
					// update program guide
					ProgramHelperV28.getInstance().processProgram( mContext, mLocationProfile, ProgramConstants.CONTENT_URI_GUIDE, ProgramConstants.TABLE_NAME_GUIDE, ops, convertV28Program( versionProgram ), tag );
					count++;
					...
					if( !inError && null != versionProgram.getRecording() ) {
						...
						if( versionProgram.getRecording().getRecordId() > 0 ) {
							...
							RecordingHelperV28.getInstance().processRecording( mContext, mLocationProfile, ops, RecordingConstants.ContentDetails.UPCOMING, convertV28Program( versionProgram ), tag );
							count++;

							// update program guide recording
							RecordingHelperV28.getInstance().processRecording( mContext, mLocationProfile, ops, RecordingConstants.ContentDetails.GUIDE, convertV28Program( versionProgram ), tag );
							count++;
...
//						Log.i( TAG, "load : applying batch for '" + count + "' transactions, processing programs" );
	...
	private org.mythtv.services.api.v028.beans.Program convertV28Program( org.mythtv.services.api.v028.status.beans.Program versionProgram ) {
		...
		if( null != versionProgram.getRecording() ) {
			program.setRecording( convertV28Recording( versionProgram.getRecording() ) );
		}
		
		if( null != versionProgram.getChannelInfo() ) {
			program.setChannel( convertV28Channel( versionProgram.getChannelInfo() ) );
		}
