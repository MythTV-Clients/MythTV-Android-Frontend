 ...
 *
 */
public class LiveStreamHelperV25 extends AbstractBaseHelper {

	private static final String TAG = LiveStreamHelperV25.class.getSimpleName();
	...
	private static LiveStreamHelperV25 singleton;
	
	/**
	 * Returns the one and only LiveStreamHelperV25. init() must be called before 
	 * any 
	 * @return
	 */
	public static LiveStreamHelperV25 getInstance() {
		if( null == singleton ) {
			
			synchronized( LiveStreamHelperV25.class ) {

				if( null == singleton ) {
					singleton = new LiveStreamHelperV25();
				}
	...
	public boolean create( final Context context, final LocationProfile locationProfile, final Integer channelId, final DateTime startTime ) {
		...
		if( null != selectedPlaybackProfile ) {
			...
			try {
				Program program = RecordedHelperV25.getInstance().findRecorded( context, locationProfile, channelId, startTime );
