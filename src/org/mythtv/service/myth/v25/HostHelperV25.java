 ...
 *
 */
public class HostHelperV25 extends AbstractBaseHelper {

	private static final String TAG = HostHelperV25.class.getSimpleName();
	
	private static final ApiVersion mApiVersion = ApiVersion.v025;
	
	private static MythServicesTemplate mMythServicesTemplate;

	private static HostHelperV25 singleton;
	
	/**
	 * Returns the one and only HostHelperV25. init() must be called before 
	 * any 
	 * @return
	 */
	public static HostHelperV25 getInstance() {
		if( null == singleton ) {
			
			synchronized( HostHelperV25.class ) {

				if( null == singleton ) {
					singleton = new HostHelperV25();
				}
