 ...
 *
 */
public class RecordedHelperV28 extends AbstractBaseHelper {
	...
	private static RecordedHelperV28 singleton;
	
	private final ChannelDaoHelper mChannelDaoHelper = ChannelDaoHelper.getInstance();
	
	/**
	 * Returns the one and only RecordedHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static RecordedHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( RecordedHelperV28.class ) {

				if( null == singleton ) {
					singleton = new RecordedHelperV28();
				}
	...
	public Integer countRecordedByTitle( final Context context, final LocationProfile locationProfile, String title ) {
		...
		Integer count = ProgramHelperV28.getInstance().countProgramsByTitle( context, locationProfile, ProgramConstants.CONTENT_URI_RECORDED, ProgramConstants.TABLE_NAME_RECORDED, title );
...
//						Log.v( TAG, "deleteRecorded : programGroup found" );
	...
	private int load( final Context context, final LocationProfile locationProfile, final Program[] programs ) throws RemoteException, OperationApplicationException {
		...
		if( null == context ) 
			throw new RuntimeException( "RecordedHelperV28 is not initialized" );
		...
		ProgramHelperV28.getInstance().findAllPrograms( context, locationProfile, ProgramConstants.CONTENT_URI_RECORDED, ProgramConstants.TABLE_NAME_RECORDED );
		...
		ProgramHelperV28.getInstance().deletePrograms( context, locationProfile, ProgramConstants.CONTENT_URI_RECORDED, ProgramConstants.TABLE_NAME_RECORDED, tag );
//		RecordingHelperV28.getInstance().deleteRecordings( context, locationProfile, ops, RecordingConstants.ContentDetails.RECORDED, lastModified );
