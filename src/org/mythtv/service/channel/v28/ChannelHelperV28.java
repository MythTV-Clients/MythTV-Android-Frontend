 ...
 * 
 */
package org.mythtv.service.channel.v28;
 ...
 *
 */
public class ChannelHelperV28 extends AbstractBaseHelper {
	...
	private static ChannelHelperV28 singleton;
	
	/**
	 * Returns the one and only ChannelHelperV28. init() must be called before 
	 * any 
	 * @return
	 */
	public static ChannelHelperV28 getInstance() {
		if( null == singleton ) {
			
			synchronized( ChannelHelperV28.class ) {

				if( null == singleton ) {
					singleton = new ChannelHelperV28();
				}
...
//		Log.d( TAG, "findChannel : exit" );
	...
	private ChannelInfo[] downloadChannels( final Context context, final LocationProfile locationProfile, final int sourceId ) throws MythServiceApiRuntimeException {
		...
		ResponseEntity<ChannelInfoList> responseEntity = mMythServicesTemplate.channelOperations().getChannelInfoList( sourceId, 0, null, etag );
	...
	private int load( final Context context, final LocationProfile locationProfile, final List<ChannelInfo> allChannelsList ) throws RemoteException, OperationApplicationException {
		...
		if( null == context ) 
			throw new RuntimeException( "ChannelHelperV28 is not initialized" );
