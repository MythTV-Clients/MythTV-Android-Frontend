 ...
 * 
 */
package org.mythtv.service.channel.v25;
 ...
 *
 */
public class ChannelHelperV25 extends AbstractBaseHelper {

	private static final String TAG = ChannelHelperV25.class.getSimpleName();
	
	private static final ApiVersion mApiVersion = ApiVersion.v025;
	
	private static MythServicesTemplate mMythServicesTemplate;
	
	private static ChannelHelperV25 singleton;
	
	/**
	 * Returns the one and only ChannelHelperV25. init() must be called before 
	 * any 
	 * @return
	 */
	public static ChannelHelperV25 getInstance() {
		if( null == singleton ) {
			
			synchronized( ChannelHelperV25.class ) {

				if( null == singleton ) {
					singleton = new ChannelHelperV25();
				}
...
//		Log.d( TAG, "findChannel : exit" );
	...
	private int load( final Context context, final LocationProfile locationProfile, final List<ChannelInfo> allChannelsList ) throws RemoteException, OperationApplicationException {
		...
		if( null == context ) 
			throw new RuntimeException( "ChannelHelperV25 is not initialized" );
